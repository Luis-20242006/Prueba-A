...
# SUBIR LA BASE DE DATOS EN LA PÁGINA WEB
st.subheader("📂 Configuración del Juego")
archivo_subido = st.file_uploader("Sube tu base de datos en formato Excel (.xlsx)", type=["xlsx"])
...
if archivo_subido is not None:
    try:
        ...
        if not niveles:
            st.error("❌ No se encontraron niveles válidos ('Fácil', 'Intermedio', 'Difícil') en el Excel.")
            st.stop()
        ...
        st.success(f"✅ Nivel seleccionado: {nivel_elegido}. ¡Todo listo para jugar!")
        st.markdown("---")
    ...
    st.info("👋 Por favor, sube un archivo Excel para cargar las preguntas y empezar a jugar.")
